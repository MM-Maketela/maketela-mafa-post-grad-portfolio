...
export const PortfolioCardWeb: React.FC<WebCardProps> = ({ name, image, repoLink, usedTools, numberOfDevs }) => {
  return (
        ...
        <div style={{ display: "flex" }}>
          <ul className={classes.listOfTools}>
            {usedTools.map((element, index) => (
              <li id={index.toString()} className={classes.tool}>
                {element}
              </li>
            ))}
          </ul>
          <a href="" style={{ flexGrow: "1", display: "flex", gap: ".2rem", justifyContent: "flex-end", alignItems: "center" }}>
            <h4>CODE</h4>
